...
class NonCdbService(DbService):
    
    CSV_OUTPUT_PATH:str = 'staging/noncdb'
    ...
    def __init__(self, repo_factory: ApplicationFactory, max_threads:int = cff_loader.appConfig.MaxThread):        
        self._nonCdbRepository = repo_factory.nonCdbRepository()        
        self._multiThreads = MultiThreads(max_threads)
    
    @property
    def repository(self):
        return self._nonCdbRepository
    ...
    def executing(self, dbid:int64, begin_snap:int64, end_snap:int64):
        try:
            functions = [
                ...
                ('getAshOverallOperation',[dbid, begin_snap, end_snap], {}),
                ('getAshOverallSqlEvent',[dbid, begin_snap, end_snap], {}),
                ...
                ('getAwrAshOverallWaitClass',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallEvent',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallModule',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallProgram',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallSegment',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallIndex',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallTable',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallSql',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallSqlCpu',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallSqlIo',[dbid, begin_snap, end_snap], {}),
                ('getAwrAshOverallSqlWait',[dbid, begin_snap, end_snap], {}),
    ...
    # def __processing(self, functions):
    #     '''Processing Cdb'''
    #     def result_callback(fn_name, result):
    #         try:
    #             data = [res.__dict__ for res in result if res is not None]                
    #             pd.DataFrame(data).to_csv(f"{NonCdbService.CSV_OUTPUT_PATH}/{fn_name}.csv", index=False)
    #         except Exception as ex:
    #             print(f"{fn_name} --> err: {ex}")
    #     try:            
    #         tasks = [
    #             (getattr(self.cdbRepository, fn_name), args, kwargs)
    #             for fn_name, args, kwargs in functions
    #         ]
    #         self.multiThreads.execute(tasks, callback=result_callback)
    #     except Exception as ex:
    #         self._logger.error(f'processing error: {ex}')
